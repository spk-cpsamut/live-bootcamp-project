...
pub async fn signup(
    ...
    let password = HashedPassword::parse(payload.password).await.map_err(|_| AuthAPIError::InvalidCredentials)?;
    ...
    let _ = user_store_write.add_user(user).await.map_err(|e| AuthAPIError::UnexpectedError(e.into()));
