...
impl UserStore for PostgresUserStore {
    ...
    async fn get_user(&self, email: &Email) -> Result<User, UserStoreError> {
        ...
        let email = Email::parse(user.email.into())
            .map_err(|e| UserStoreError::UnexpectedError(eyre!("unexpected email error")))?;
        let hashed_password = HashedPassword::parse_password_hash(user.password_hash.into())
            .map_err(|e| UserStoreError::UnexpectedError(eyre!(e)))?;
